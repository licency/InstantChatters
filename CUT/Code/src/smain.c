...
void *handle_client(void *arg)
{
  ...
  char ch ;
  printf("%s wants to join y or n : ",cli->name);
  scanf("%c",&ch); getchar();
  
  if(ch=='n')
  {
    sprintf(buff_out, "%s has been denied joining \n", cli->name);
		printf("%s", buff_out);
		send_rejection_message(buff_out, cli->uid); //new func 
    close(cli->sockfd);
  ...
  {
    sprintf(buff_out,"%s welcome to chatroom \n",cli->name );
    printf("%s",buff_out);
  }
...
int main(int argc, char **argv)
{
	...
	while(1)
  {
		...
		connfd = accept(listenfd, (struct sockaddr*)&cli_addr, &clilen);  //accept new client 
